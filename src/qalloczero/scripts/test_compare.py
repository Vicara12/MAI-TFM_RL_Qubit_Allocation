...
def validate():
  ...
  algos = dict(
    ...
    da_seq  = DirectAllocator.load("trained/da_v13",    device="cuda", checkpoint=-1).set_mode(DirectAllocator.Mode.Sequential),
    da_par  = DirectAllocator.load("trained/da_v13",    device="cuda", checkpoint=-1).set_mode(DirectAllocator.Mode.Parallel),
...
def benchmark():
  ...
  algos = dict(
    # hqa = HQA(lookahead=True, verbose=True),
    da_sequential = DirectAllocator.load("trained/da_v13", device="cuda", checkpoint=-1).set_mode(DirectAllocator.Mode.Sequential),
    da_parallel   = DirectAllocator.load("trained/da_v13", device="cuda", checkpoint=-1).set_mode(DirectAllocator.Mode.Parallel),
