...
def validate():
  ...
  algos = dict(
    da_fast = DirectAllocator.load("trained/da",    device="cuda", checkpoint=check_n).set_mode(DirectAllocator.Mode.Fast),
    da_seq  = DirectAllocator.load("trained/da",    device="cuda", checkpoint=check_n).set_mode(DirectAllocator.Mode.Sequential),
    da_par  = DirectAllocator.load("trained/da",    device="cuda", checkpoint=check_n).set_mode(DirectAllocator.Mode.Parallel),
...
def benchmark():
  ...
  algos = dict(
    da_fast       = DirectAllocator.load("trained/da", device="cuda", checkpoint=check_n).set_mode(DirectAllocator.Mode.Fast),
    da_sequential = DirectAllocator.load("trained/da", device="cuda", checkpoint=check_n).set_mode(DirectAllocator.Mode.Sequential),
    da_parallel   = DirectAllocator.load("trained/da", device="cuda", checkpoint=check_n).set_mode(DirectAllocator.Mode.Parallel),
    # azero =               AlphaZero.load("trained/da_v2_ft", device="cpu"),
  )
  cfg = TSConfig(
    target_tree_size=512,
